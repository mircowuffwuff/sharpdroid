...
**a uri names a folder this app must already be able to reach** — one the user added under Game files, or any folder while all-files access is held. a uri permission the sending app attaches covers the document it names and **not** the documents inside it, so it is not what makes the form work: the eboot and `param.json` are children, and the platform refuses them. the path form carries no such condition, which is why it is the one to prefer where a caller has both. [`frontends.md`](frontends.md) is the whole of that contract, and it is the page to send somebody integrating against this app.
